function output = F_subset_OMHCHO_download(inp)
% subset OMHCHO downloaded from nasa ges disc. by Kang Sun on 2018/05/16
...
for iday = 1:nday
    ...
    for ifile = 1:length(he5fn)
        ...
        xtrackmask;
        if sum(validmask(:)) > 0
            disp(['You have ',sprintf('%5d',sum(validmask(:))),' valid L2 pixels on orbit ',he5fn(ifile).name(36:40),'.']);
        end
