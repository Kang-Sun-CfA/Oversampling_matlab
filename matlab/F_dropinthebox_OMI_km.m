function output_oversample = F_dropinthebox_OMI_km(inp,output_subset)
...
% Modified from F_oversample_OMI_km.m by Kang Sun on 2018/07/20
...
% define x y mesh for boundaries
...
nL2 = sum(validmask);
if nL2 <= 0;return;end
disp(['Dropping pixels from ',datestr([Startdate 0 0 0]),' to ',...
...
for irow = 1:nrows
    for icol = 1:ncols
            ...
            ind = find(abs(x-xcenter) <= res/2 & abs(y-ycenter) <= res/2);
%             xp = [xgridr(icol), xgridr(icol), xgridr(icol+1), xgridr(icol+1), xgridr(icol)];
%             yp = [ygridr(irow), ygridr(irow+1), ygridr(irow+1), ygridr(irow), ygridr(irow)];
%             ind = inpolygon(x,y,xp,yp);
%             
