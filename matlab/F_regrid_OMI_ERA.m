function output_regrid = F_regrid_OMI_ERA(inp,output_subset)

% Modified from F_regrid_OMI_ERA.m by Kang Sun on 2017/01/15. Instead of
% NARR wind, use ERA wind, downloaded by fetch_ERA.m

if ~isfield(inp,'ERA_download_dir')
    error('You have to know where ERA data are!')
else
    ERA_download_dir = inp.ERA_download_dir;
end
...
% saved ERA data in which hours
ERA_hour_vec = 0:6:24;
% % work out ERA's map projection
% mstruct_ERA = defaultm('lambertstd');
% mstruct_ERA.origin = [50 360-107 0];
% mstruct_ERA.mapparallels = 50;
% mstruct_ERA.nparallels = 1;
% mstruct_ERA.scalefactor = 6367470/1e3;
% mstruct_ERA.falseeasting = 0;
% mstruct_ERA.falsenorthing = 0;
% mstruct_ERA = defaultm(mstruct_ERA);
% % map center lat lon to ERA projection x y
% [x_c, y_c] = mfwdtran(mstruct_ERA,clat,clon);

totalindex = 1:nL2;

% work out the wind interpolation day-by-day
disp('Interpolating ERA wind to satellite pixel locations...')

for ERA_date = datenum(day_1(1:3)):1:datenum(day_end(1:3))
    int = UTC > ERA_date & UTC <= ERA_date+1;
    if sum(int) > 0
        disp(['Interpolating ERA for ',datestr(ERA_date)])
        lon_interp_day = Lon_c(int);
        lat_interp_day = Lat_c(int);
        utc_interp_day = UTC(int);
        idx_interp_day = totalindex(int);
        loadstart = find(datenum(ERA_date)+ERA_hour_vec/24 <= min(utc_interp_day),1,'last');
        loadend = find(datenum(ERA_date)+ERA_hour_vec/24 >= max(utc_interp_day),1,'first');
        inp_interp_era = [];
        inp_interp_era.hour = ERA_hour_vec(loadstart:loadend);
        inp_interp_era.datenum = ERA_date;
        inp_interp_era.lon = lon_interp_day;
        inp_interp_era.lat = lat_interp_day;
        inp_interp_era.utc = utc_interp_day;
        inp_interp_era.ERA_download_dir = ERA_download_dir;
        outp_interp_era = F_interp_era(inp_interp_era);
        U_sat_pbl(idx_interp_day) = outp_interp_era.u;
        V_sat_pbl(idx_interp_day) = outp_interp_era.v;
        T_sat_pbl(idx_interp_day) = outp_interp_era.T2m;
    end
end
