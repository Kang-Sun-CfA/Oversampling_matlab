function [Sum_Above,Sum_Below,Pixels_count] = ...
    F_cakeCut(Sum_Above,Sum_Below,Pixels_count,...
    Lon_r,Lat_r,Lon_c,Lat_c,Lon_left,Lat_low,VCD,VCD_Unc,Res,options)
% matlab function to cut arbitrary polygon into horizontal and then
% vertical small pieces. Use V/HcakeCut algorithms written by Kai Yang
% written by Kang Sun on 2017/05/06

% updated by Kang Sun on 2017/05/07 to add pixel inflation and super
% gaussian spatial response function (SRF)
...
Sum_Above = Sum_Above+Sub_Area/pixel_area/VCD_Unc*VCD;
Sum_Below = Sum_Below+Sub_Area/pixel_area/VCD_Unc;
%         Sum_Above = Sum_Above(row,col) + temp_area/A/VCD_Unc(1)*VCD(1);
%         Sum_Below = Sum_Below(row,col) + temp_area/A/VCD_Unc(1);
