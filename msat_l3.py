...
def run_popy(l2_dir: str, output_dir: str, m: popy, ncores=8: int, block_length=300: int):
...
def flag_empty_args(args):
    if not args.l2_input_dir:
        raise ValueError("--l2-input-dir must be set")
    ...
    if not args.end_time:
        raise ValueError("--end-time must be set")
...
def main():
    ...
    group = parser.add_argument('-i', '--l2-input-dir', help='input directory')
    group = parser.add_argument('-o', '--output-dir', help='output directory')
    group = parser.add_argument('--start-time', help='ISO-8601 start time')
    group = parser.add_argument('--end-time', help='ISO-8601 end time')
    group = parser.add_argument(
        '-w', '--west', type=float, help='grid west boundary')
    group = parser.add_argument(
        '-e', '--east', type=float, help='grid east boundary')
    group = parser.add_argument(
        '-n', '--north', type=float, help='grid north boundary')
    group = parser.add_argument(
        '-s', '--south', type=float, help='grid south boundary')
    group = parser.add_argument(
    ...
    run_popy(l2_dir=args.l2_input_dir, output_dir=args.output_dir, m=m,
             num_cores=args.num_cores, block_length=args.block_length)
